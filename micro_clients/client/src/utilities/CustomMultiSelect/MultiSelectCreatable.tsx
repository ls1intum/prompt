...
export const MultiSelectCreatable = ({
  ...
  const exactOptionMatch = data.some(
    (item) =>
      item.label.trim().toLocaleLowerCase() === search.trim().toLocaleLowerCase() ||
      item.value.trim().toLocaleLowerCase() === search.trim().toLocaleLowerCase(),
  )
    ...
    if (val === '$create') {
      ...
      onChange(
        value.includes({ label: val, value: val })
          ? value.filter(
              (v) =>
                v.label.trim().toLocaleLowerCase() !== val &&
                v.value.trim().toLocaleLowerCase() !== val,
            )
  ...
  const handleValueRemove = (val: string): void =>
    onChange(value.filter((v) => v.value.trim().toLocaleLowerCase() !== val))
